...
const UploadPage = () => {
  ...
  // Memoize loadParticipant using useCallback to prevent unnecessary re-renders
  const loadParticipant = useCallback(async () => {
    const res = await fetch(`/api/participants/hash/${id}`);
    const data = await res.json();
    setParticipant(data);
  }, [id]); // Dependency is id, to reload if the id changes

  useEffect(() => {
    loadParticipant();
  }, [loadParticipant]); // Add loadParticipant as a dependency
          ...
          <div>
            <h3>
              ...
              <span className="font-bold">
                IKARKP24-{participant?.id.toString().padStart(6, '0')}
              </span>
            </h3>
            <label
              htmlFor="file"
